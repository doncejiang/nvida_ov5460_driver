...
/* ov5640 - sensor parameter limits */
#define ov5640_MIN_GAIN				0x0000
#define ov5640_MAX_GAIN				0x00e8
#define ov5640_MIN_FRAME_LENGTH			0x0100
#define ov5640_MAX_FRAME_LENGTH			0xffff
#define ov5640_MIN_COARSE_EXPOSURE		0x0001
#define ov5640_MAX_COARSE_DIFF			0x0004

/* ov5640 sensor register address */
#define ov5640_MODEL_ID_ADDR_MSB		0x300A
#define ov5640_MODEL_ID_ADDR_LSB		0x300B
#define ov5640_GAIN_ADDR			0x0157
#define ov5640_FRAME_LENGTH_ADDR_MSB		0x0160
#define ov5640_FRAME_LENGTH_ADDR_LSB		0x0161
#define ov5640_COARSE_INTEG_TIME_ADDR_MSB	0x015a
#define ov5640_COARSE_INTEG_TIME_ADDR_LSB	0x015b
#define ov5640_FINE_INTEG_TIME_ADDR_MSB		0x0388
#define ov5640_FINE_INTEG_TIME_ADDR_LSB		0x0389
...
static inline int ov5640_write_reg(struct camera_common_data *s_data,
	u16 addr, u8 val)
{
	int err = 0;
...
static int ov5640_write_table(struct ov5640 *priv, const ov5640_reg table[])
{
	return regmap_util_write_table_8(priv->s_data->regmap, table, NULL, 0,
		ov5640_TABLE_WAIT_MS, ov5640_TABLE_END);
}

static int ov5640_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	/* ov5640 does not support group hold */
	return 0;
}
...
static int ov5640_power_on(struct camera_common_data *s_data)
{
	...
	if (pw->reset_gpio) {
		if (gpio_cansleep(pw->reset_gpio))
			gpio_set_value_cansleep(pw->reset_gpio, GPIO_UNSET);
		else
			gpio_set_value(pw->reset_gpio, GPIO_UNSET);
	}
...
skip_power_seqn:
	if (pw->reset_gpio) {
		if (gpio_cansleep(pw->reset_gpio))
			gpio_set_value_cansleep(pw->reset_gpio, GPIO_SET);
		else
			gpio_set_value(pw->reset_gpio, GPIO_SET);
	}
...
static struct camera_common_pdata *ov5640_parse_dt(
	struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	...
	dev_dbg(dev, "reset gpio index %d\n", gpio);
...
static int ov5640_set_mode(struct tegracam_device *tc_dev)
{
	...
	err = ov5640_write_table(priv, mode_table[ov5640_MODE_COMMON]);
	if (err)
		return err;
...
static int ov5640_start_streaming(struct tegracam_device *tc_dev)
{
	...
	return ov5640_write_table(priv, mode_table[ov5640_START_STREAM]);
}
...
static int ov5640_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	err = ov5640_write_table(priv, mode_table[ov5640_STOP_STREAM]);
...
static int ov5640_board_setup(struct ov5640 *priv)
{
	...
	//err = ov5640_get_fine_integ_time(priv, &priv->fine_integ_time);
	//if (err)
...
MODULE_DESCRIPTION("Media Controller driver for Sony ov5640");
